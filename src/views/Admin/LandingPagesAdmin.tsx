...
import { normalizeLandingSlug, validateLandingPageForPublish, validateLandingSlug } from '../../lib/landing/landingValidation';
  ...
  return <div className="space-y-5">
    ...
    {isFormOpen && <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/85 p-3 backdrop-blur-sm" onMouseDown={closeForm}><form onSubmit={handleSave} onMouseDown={(event) => event.stopPropagation()} className="max-h-[94vh] w-full max-w-4xl overflow-y-auto border border-gold-dark/30 bg-[#0d0d0d] shadow-2xl"><div className="sticky top-0 z-10 flex items-center justify-between border-b border-white/10 bg-[#0d0d0d] p-5"><div><p className="text-[9px] font-bold uppercase tracking-[.2em] text-gold-light">Thông tin cơ bản</p><h3 className="mt-1 text-lg font-black uppercase text-white">{isEditing ? 'Sửa Landing Page' : 'Tạo Landing Page nháp'}</h3></div><button type="button" disabled={saving} onClick={closeForm} className="p-2 text-gray-500 hover:text-white"><X className="h-5 w-5" /></button></div>
      <div className="grid gap-5 p-5 md:grid-cols-2">
        ...
        <div className="space-y-4 border border-white/10 p-4 md:col-span-2"><h4 className="text-xs font-black uppercase text-gold-light">SEO & Social</h4><div className="grid gap-4 md:grid-cols-2"><label className="space-y-2 text-[10px] font-bold uppercase text-gray-400">SEO title<input value={form.seo.title || ''} onChange={(event) => setForm({ ...form, seo: { ...form.seo, title: event.target.value } })} className="w-full border border-white/10 bg-black px-3 py-3 text-base normal-case text-white outline-none md:text-sm" /></label><label className="space-y-2 text-[10px] font-bold uppercase text-gray-400">Ảnh social<input value={form.seo.image || ''} onChange={(event) => setForm({ ...form, seo: { ...form.seo, image: event.target.value } })} placeholder="URL ảnh Cloudinary" className="w-full border border-white/10 bg-black px-3 py-3 text-base normal-case text-white outline-none md:text-sm" /></label><label className="space-y-2 text-[10px] font-bold uppercase text-gray-400 md:col-span-2">SEO description<textarea rows={3} value={form.seo.description || ''} onChange={(event) => setForm({ ...form, seo: { ...form.seo, description: event.target.value } })} className="w-full resize-none border border-white/10 bg-black px-3 py-3 text-base normal-case text-white outline-none md:text-sm" /></label></div></div>
